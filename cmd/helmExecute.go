...
func getAndRenderImageInfo(config helmExecuteOptions, rootPath string, utils kubernetes.DeployUtils) error {
	...
	// ToDo: use the functionality from PR #1234 once it is merged
	...
	for _, valuesFile := range valuesFiles {
		...
		// ToDo: use the functionality from PR #1234 once it is merged
		tmpl, err := template.New("new").Funcs(funcMap).Parse(string(cpeTemplate))
		if err != nil {
			return fmt.Errorf("failed to parse template: %w", err)
		}
		var generated bytes.Buffer
		err = tmpl.Execute(&generated, params)
		if err != nil {
