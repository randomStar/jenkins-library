...
// commit: tests: add usage of t.TempDir for creating temporary dir
...
`)
	...
	tt := []struct {
		...
		},
		{
			...
			defaultValueFile: defaultValueFile,
			config:           config,
			expectedErr:      fmt.Errorf("failed to parse template: template: new:4: unclosed action started at new:3"),
	...
	for _, test := range tt {
		t.Run(test.name, func(t *testing.T) {
			...
			err := getAndRenderImageInfo(test.config, tmpDir, utils)
