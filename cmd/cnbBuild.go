...
func callCnbBuild(config *cnbBuildOptions, telemetryData *telemetry.CustomData, utils cnbutils.BuildUtils, commonPipelineEnvironment *cnbBuildCommonPipelineEnvironment, httpClient piperhttp.Sender) error {
	stepName := "cnbBuild"
	cnbTelemetry := buildpacks.NewBuildpacksTelemetry(telemetryData)
	...
	cnbTelemetry.WithImage(dockerImage)
	...
	return cnbTelemetry.Export()
}

func runCnbBuild(config *cnbBuildOptions, cnbTelemetry buildpacks.BuildpacksTelemetry, utils cnbutils.BuildUtils, commonPipelineEnvironment *cnbBuildCommonPipelineEnvironment, httpClient piperhttp.Sender) error {
	...
	telemetrySegment := prepareTelemetry(config, utils)
	...
	cnbTelemetry.AddSegment(telemetrySegment)
...
func prepareTelemetry(config *cnbBuildOptions, utils cnbutils.BuildUtils) buildpacks.Segment {
